...
def test_mpd_tag(input_file):
    ...
    with open(input_file, mode="r", encoding="UTF-8") as manifest_file:
        ...
        assert mpd.media_presentation_duration == timedelta(minutes=9, seconds=55.46)
        assert mpd.profiles == "urn:mpeg:dash:profile:isoff-on-demand:2011"
        assert len(mpd.program_informations) == 1
        assert (
            mpd.program_informations[0].titles[0].text
            == "dashed/BigBuckBunny_1s_onDemand_2014_05_09.mpd generated by GPAC"
        )
        assert (
            mpd.program_informations[0].more_info_url == "http://gpac.sourceforge.net"
        )
        assert len(mpd.periods) == 1
        assert mpd.periods[0].id is None
        assert mpd.periods[0].unparsed_attr("duration") == "PT0H9M55.46S"
...
def test_parse_from_file_mpd_tag(input_file):
    ...
    assert len(mpd.periods) == 1
    assert mpd.periods[0].id is None
    assert mpd.periods[0].duration == timedelta(minutes=9, seconds=55.46)
...
def test_parse_from_url_mpd_tag(url):
    ...
    assert mpd.media_presentation_duration == timedelta(
    ...
    assert mpd.publish_time == datetime.fromisoformat("2015-08-04T10:47:32.000Z")
